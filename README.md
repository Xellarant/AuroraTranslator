...
- committed character-state fixtures now cover baseline, bond/text-choice, partial and completed early fighter flows, Monk skill/tool/language completion, classic Rogue `Expertise`, Warlock `Eldritch Invocation` plus Pact Magic spell picks, PHB 2024 Sorcerer `Metamagic`, elf darkvision, first-party Cleric `Divine Order` and Druid `Primal Order` flows, PHB 2024 replacement-style feature picks, first-party oversubscribe warning scenarios, first-party fly/climb/swim movement scenarios, and a companion burrow-speed scenario
